...
(function() {
  ...
  NWLibrary.core.prototype = {
                      ...
                      if(xss.indexOf(xpathKind[i]) > -1){
                        ...
                        if(obj === node){
                          ...
                          switch(xpathKind[i]){
                            ...
                            case "$=" : if (NWLibrary.exec("/"+searchSelector+"$/i").test(attVal)) results.push(node); break;
                            case "^=" : if (NWLibrary.exec("/^"+searchSelector+"/i").test(attVal)) results.push(node); break;
                            case "~=" : if (NWLibrary.exec("/\\b"+searchSelector+"\\b/i").test(attVal)) results.push(node); break;
                          }
                        }
                        break;
                      }
    ...
    parent : function() {
      ...
      if(cloneNW.el.nodeName)
        cloneNW.el = cloneNW.el.parentNode;
     	else
     	  throw "element count is not 1";
     	return cloneNW;
    },
    prevnode : function() {
      var cloneNW = NWLibrary.copy(this);
        ...
        childs.each(function(i){
          ...
          if(type !== "button" && type !== "submit"){
            if(/(checkbox|radio)/gi.test(type)) {
              if(obj.checked) arrFormData.push({name:obj.name, value:encodeURIComponent(obj.value)}); 
            }else{
              arrFormData.push({name:obj.name, value:encodeURIComponent(obj.value)});
            }    
          }
        });  
      });
  ...
  NWLibrary.extend("ajax", function() {
    ...
    for (var attr in arguments[0]) {
      if (NWLibrary.isString(arguments[0][attr])) {
      	if(attr != "url" && attr != "param")
	        vars[attr] = (arguments[0][attr]).toLowerCase();
        else
  ...
  NWLibrary.extend("delScript", function(src) {
    ...
    for (var i = 0; i < scripts.length; i++) {
      if (scripts[i].src.indexOf(src) > -1) {
        NWLibrary("head").remove(scripts[i]);
        break;
      }
    }
  });
  ...
  (function() {
    ...
    for (var i = 0; i < arrData.length; i++) {
      if (arrData[i].exp.test(userAgent) === true) {
        ...
        version = (browser == 'ie') ? userAgent.match(arrData[i].ver)[0].split(" ")[1] : userAgent.match(arrData[i].ver)[0].split("/")[1];
        break;
      }
    }
    if (browser == '') {
